...
class ItempediaApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("RoR2 Itempedia - Энциклопедия предметов Risk of Rain 2")
    ...
    def clear_items(self):
        # очищает все предметы из таблицы и csv файла
    ...
    def create_menu(self):
        ...
        import_action = file_menu.addAction('Импорт предметов')
        import_action.triggered.connect(self.import_items)

        export_action = file_menu.addAction('Экспорт предметов')
    ...
    def load_items(self):
        ...
        try:
            ...
            if Path(csv_path).exists():
                ...
                # создаем демо данные если файла нет
    ...
    def save_items(self):
        # сохраняет предметы в csv файл
        try:
            with open('items.csv', 'w', encoding='utf-8', newline='') as file:
                writer = csv.DictWriter(file, fieldnames=['name', 'rarity', 'desc', 'effect'])
                writer.writeheader()
            ...
            QMessageBox.critical(self, "Ошибка", f"Не удалось сохранить предметы: {str(e)}")
    ...
    def export_items(self):
        # экспорт всех предметов в csv-файл
        ...
        if file_path:
            try:
                ...
                self.statusBar().showMessage(f"Экспортировано предметов: {len(self.items)}")
...
def main():
    ...
    app.setApplicationName("RoR2 Itempedia")
